...
const MenuAuthorization = forwardRef((props, ref) => {
    ...
    const queryMenuTreeList = (item = null) => {
        ...
        request.queryMenuTreeList().then(res => {
            if (res.data.code == 200) {
                let list = setTreeList(res.data.data.treeList)
                // 超管禁止选择系统管理下属菜单（默认选中，禁止删掉admin系统权限）
                // 获取首页名称
                const menulocal = JSON.parse(localStorage.getItem(window.envConfig['ROOT_APP_INFO']))?.menuList
                const menuindexname = menulocal?menulocal[0].title:'首页'
                let menu = [menuindexname, "系统管理", "管理员", "角色管理", "菜单管理", ]
                ...
                setTreeData(list)
                setExpandedKeys(res.data.data.ids) //ids返回所有菜单key
                setAllTreeKeys(res.data.data.ids)
...
const queryRolePermission = (item) => {
    ...
    request.queryRolePermission({ roleId: item.id }).then(res => {
        setLoading(false);
        if (res.data.code === 200) {
            // 1. 提取数组中的 id 字段，转成字符串（如 ["id1", "id2"] → "id1,id2"）
            const permissionIds = res?.data?.data.map(item => item.id);
            
            // 2. 设置到 checkedKeys（用于树形控件选中）
            setCheckedKeys(permissionIds); // 如果树控件需要数组格式
            
            // 3. 设置 lastpermissionIds（用于提交时对比）            
            setLastpermissionIds(permissionIds); // 存储字符串格式，如 "id1,id2"
        }
    });
};
