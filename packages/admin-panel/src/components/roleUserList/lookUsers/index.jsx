...
const LookUsers = forwardRef((props, ref) => {
    ...
    const getUserlist = (item) => {
        ...
        request.userRoleList(params).then(res => {
            if (res.data.code == 200) {
                setDataUserList(res.data.data.records)
    ...
    const rowSelection = {
        ...
        getCheckboxProps: (record) => ({
            disabled: (record.roles&&record.roles[0]?.roleCode !== details.id)||dataUserList.some(item=>item.id==record.id)
        }),
    };
    // 添加用户到当前角色
    ...
    return (
        <>
            <Drawer title="查看用户" open={lookVisible} closable={false} onClose={() => { setLookVisible(false);setSelectedRowKeys([]) }} width='40%'
