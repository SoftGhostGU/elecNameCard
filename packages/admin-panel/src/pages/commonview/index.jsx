...
const ISROUTER = window.envConfig['ISROUTER']
const Commonview = () => {
  ...
  useEffect(() => {
    ...
    if (ISROUTER) {
      ...
      // 标签栏
      if (menuList) routerFun(setmenu(menuList))
    } else routerFun(routeslist)
  }, [location])
  useEffect(() => {
    const { menuList } = JSON.parse(localStorage.getItem(window.envConfig['ROOT_APP_INFO']))
    if (ISROUTER) {
      if (menuList) {
        ...
        const menulocal = JSON.parse(localStorage.getItem(window.envConfig['ROOT_APP_INFO']))?.menuList
        const menuindexname = menulocal.length != 0 ? menulocal[0].meta.title : '首页'
        menuList.map(item => {
          if (item.label == menuindexname) {
            setRouteList([{
              ...item,
              colordisabled: 'true'
            }])
  ...
  useEffect(() => {
    // 判断是否是登录页进入首页
    const Jiaxiaohan = localStorage.getItem("ZhuXiaoJia");
    if (Jiaxiaohan) {
  ...
  function routerFun(array) {//递归
    for (let index = 0; index < array.length; index++) {
      ...
      if (location.pathname == element?.key) {
        ...
        const menulocal = JSON.parse(localStorage.getItem(window.envConfig['ROOT_APP_INFO']))?.menuList
        const menuindexname = menulocal.length != 0 ? menulocal[0].meta.title : '首页'
        document.title = element.label == menuindexname ? window.envConfig.ROOT_APP_NAME : element.label + " - " + window.envConfig.ROOT_APP_NAME
        setRouteList((e) => {
  ...
  const tabBar = ({ key }) => {
    ...
    switch (key) {
      case "1"://关闭当前 
        if (e.label != menuindexname) delRouter(e, i)
        break;
      case "2": // 关闭其他
        //保存首页
  ...
  const closeLeftMenu = (e) => {
    ...
    const menuindexname = menulocal.length != 0 ? menulocal[0].meta.title : '首页'
    if (index !== -1) {
      const home = routeList.filter(obj => obj.label == menuindexname)
...
function Author() {
  return (
    <div className="author">
      <p> React-Ant-Admin © 2025 <a href="https://gitee.com/jiangsihan/react-admin-vite" target="_blank">React-Ant-Admin@v2.0.0  </a>
        | <a href="https://jiangsihan.cn/react-admin-vite-press" target="_blank">开发文档</a>
...
function MenuSearch(props) {
  ...
  useEffect(() => {
    ...
    if (ISROUTER) {
      ...
      resetMenu(routeslist);
      setTreeData(routeslist);
    }
