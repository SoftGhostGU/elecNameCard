...
export default function Login() {
  ...
  const onFinish = async (userInfo: any) => {
    console.log("提交的登录信息：", userInfo);
    setLoading(true)
    setSubmitLoginName('登录中...')
    // 登录逻辑
    const loginRequest = {
      username: userInfo.username,
      password: userInfo.password,
    }
    request.getLogin(loginRequest).then((res: any) => {
      console.log("登录返回结果：", res.data.data);
      if (res.data.code === 200) {
        setTimeout(() => {
          localStorage.setItem('isLoggedIn', 'true' + new Date().getTime());
  ...
  const validateInputCode = async (rule: any, value: any) => {
    if (value) {
      if (!GraphicCodeRef.current) {
        throw new Error("验证码组件未加载，请稍后再试！")
      }
  ...
  return (
    <div className="login" style={{ backgroundImage: `url(${loginBg})` }}>
      ...
      <div className="login-content-box">
        <div className="logo_left_top"><p>Visiting Card</p><p>后台管理系统</p></div>
